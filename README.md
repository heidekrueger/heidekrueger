...
In the past, I've also worked on deep learning with `TensorFlow` and `keras`, Big Data with `spark` and `scala`, and written general-purpose code in `Java`, `C#` and `C++`, as well as some simulations with `MATLAB`. I know my way around a `Unix` system and I have worked extensively with RDBMSs (`MySQL`, `Postgres`, `MS SQL`, `Oracle`) and built Dashboards and data-centric WebApps using `Tableau` or `RShiny`. I also have a fairly good overview of cloud computing resources (especially in the `AWS` and `Azure` ecosystems).
